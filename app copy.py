...
app.config['BASIC_AUTH_USERNAME'] = os.environ.get('APP_USERNAME', 'admin_absensi')
app.config['BASIC_AUTH_PASSWORD'] = os.environ.get('APP_PASSWORD', 'password_absensi_123')
...
def index():
    ...
    selected_employee_id = request.args.get('employee_id', type=int) # Ambil ID dari query param

    try:
        employees = db.query(Employee.id, Employee.name).order_by(Employee.name).all()

        # Jika ada employee_id terpilih, cari absensi terakhirnya
        if selected_employee_id:
            ...
            if last_att:
                employee = db.query(Employee).get(selected_employee_id) # Ambil nama pegawai
                last_attendance_data = {
                    "employee_id": last_att.employee_id,
                    "employee_name": employee.name if employee else "Nama Tidak Ditemukan",
                    "timestamp": last_att.timestamp.isoformat(),
                    "type": last_att.type,
                    "latitude": last_att.latitude,
                    "longitude": last_att.longitude,
                    "photo_base64": base64.b64encode(last_att.photo_blob).decode('utf-8')
                }
    except Exception as e:
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5000, debug=True) # Hapus ssl_context jika tidak pakai HTTPS
